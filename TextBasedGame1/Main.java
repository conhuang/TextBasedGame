...
{   static boolean stillPlay = true;
    ...
    public static void main(String [] args){ 
        ...
        while(stillPlay){
            ...
            String[] places = {"Small Island","Ocean","Meadow","Meadow","Swamp",
                               ...
                               "Anarchy Acres","Lonely Lodge","Forest","Forest","Greasy Grove",
                               "Wall","Tilted Towers","Lucky Landing","Retail Row","Ocean"};
            ...
            String [] foeNames = {"Connie","Sijia","Huang","Miranda","Jack Nicholson",
                    "Elon Musk", "Lexie","Eminem", "Mark","Maggie"};
            ...
            for(int i =0; i<enemies.length; i++){
                ...
                while(x==0&&y==0){
                    x = (int)(Math.random()*5); y = (int)(Math.random()*5);
                }
                player.w.place(x,y).addPerson(enemies[i]);
            }

            //creating items
            String[] itemNames={"water","shotgun","crossbow","newspaper","lantern",
                    "branch","berries","grass","trees","deer","fish", "pickaxe"};

            player.w.place(1,2).addItem(new Weapon(itemNames[1],(int)(Math.random()*30)));
            player.w.place(2,1).addItem(new Weapon(itemNames[2],(int)(Math.random()*30)));
            player.w.place(3,2).addItem(new Weapon(itemNames[5],(int)(Math.random()*30)));
            player.w.place(2,3).addItem(new Weapon(itemNames[1],(int)(Math.random()*30)));
            player.w.place(3,3).addItem(new Weapon(itemNames[2],(int)(Math.random()*30)));            
            player.w.place(1,3).addItem(new Edible(itemNames[0],5,false));
            player.w.place(0,2).addItem(new Edible(itemNames[6],0,true));
            player.w.place(0,3).addItem(new Edible(itemNames[6],0,true));
            ...
            player.w.place(1,3).addItem(new Edible(itemNames[10],10,false));
            player.w.place(2,2).addItem(new Weapon(itemNames[11],10));

            //THE GAME
            System.out.println("\f=================================");
            System.out.println("--- FORTNITE : BATTLE ROYALE --- ");
            System.out.print("=================================\n");
            ...
            while (player.isAlive){
                ...
                if(player.victories==10){
                    System.out.println("You win! After defeating all ten of the people stranded here,\n"+
                    "a team of people who have been studying you this entire time \nfly into the island"+
                    " and bring you back to civilization. \nThey even give you a juicy Double-Double"+
                    " to eat for all your hard work and determination.");
        ...
        switch(words[0].toLowerCase()){
            ...
            if(words.length>1){
                if(words.length==4 && words[2].equalsIgnoreCase("with")){
                    Foe f = p.currentPlace().getRandFoe();
                    p.attack(f,(Weapon)p.getItem(words[3]));
                    if (f.stamina>0)
                        System.out.println("Your enemy has weakened! Attack again to make a kill!");
            ...
            else if(p.currentPlace().hasItem(words[1])||p.hasItem(p.getItem(words[1]))){
                if(((Edible)(p.getItem(words[1]))).isPoisonous()){
                    p.isAlive = false;
                    System.out.println("You just ate poisonous food. In a few hours you are"+
                        "\ngoing to drop dead and your meat will be reused for McNuggets.");
                }
                else{
                    p.eat((Edible)p.currentPlace().getItem(words[1]));
                    System.out.println("Oh yeah, that hit the spot.");
                }
            }else
            ...
            case "drink":
            if(words.length==1)
                System.out.println("Drink what? I DEMAND MORE SPECIFICITY");
            else if((p.currentPlace().hasItem(words[1]) ||p.hasItem(Item.getItem(words[1])))&& (Edible)p.currentPlace().getItem(words[1]) instanceof Edible){
                p.eat((Edible)p.currentPlace().getItem(words[1]));
            ...
            if(p.currentPlace().canSleep()&& p.getStamina()<20){
                ...
                System.out.println("It's too early to sleep!");
            }
            else
                System.out.println("You can't sleep there are enemies nearby");
            ...
            default:
            System.out.print("My apologies. I don't understand your accent. Please try again, but better.\n");
    ...
    public static void fightOrFlight(String ans, Scanner scan, Player p){
        switch(ans){
            ...
            System.out.println("As quickly as you can, you run away from your opponent\n"
                +"and hide in the nearest hiding spot."+ "\n\n" + s);
            break;

            case "fight":
            System.out.print("Choose your weapon.\n>> ");
            String h = scan.nextLine();
            if(p.hasItem(p.getItem(h))){
                Foe f = p.currentPlace().getRandFoe();
                    p.attack(f,(Weapon)p.getItem(h));
                if(f.stamina>0)
                    System.out.println("Your enemy has weakened! Attack again to make a kill!"); 
            }
            else{
                System.out.print("You do not have this weapon. Choose to fight or run.\n>> ");
                ans = scan.nextLine();
